...
Where $f$ is a nonlinear function e.g. Rectifier Linear Unit (ReLU), $U$ and $W$ are weight matrices that are shared across timesteps \cite{Young}.
...
LSTMs \cite{Hochreiter1997} is a specific variant of RNNs that improve on it by introducing a 'forget' gate that regulates a cell state within the network as it allows information to be removed or added. This addition allows LSTMs to overcome the vanishing and exploding gradient problem \cite{Socher} that feedforward networks i.e. RNNs are prone to.

\subsubsection{Convolutional Neural Networks}
\label{sec:CNN}

CNNs have traditionally seen more use for image classification tasks \cite{Young}. However, with the work accomplished by Collbert et al. \cite{Collobert2011} who proposed a CNN-based model designed to solve various NLP tasks by extracting the most important n-gram features to produce a language model. The language model created by 
