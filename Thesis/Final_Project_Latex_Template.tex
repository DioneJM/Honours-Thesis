...
Two experiments have been conducted to evaluate the performance of a model. The first experiment is designed to determine the model's ability to produce the correct label on a criteria-level allowing for a more detailed insight on the reasoning for a particular article's credibility score. Whereas the second experiment is designed to determine the model's ability to correctly determine whether an article is considered to have low credibility or not for situations where the detailed insight is unnecessary.

\subsubsection{Credibility Classification Experiment}
\label{sec:CredibilityClassificationExperiment}
The Credibility Classification Experiment (CCE) aims to determine a model's ability in correctly identifying whether an article either satisfies or doesn't satisfy a certain criteria. For each criteria, the respective model was evaluated by measuring the micro averaged f1-scores of that model.


\subsubsection{Low Credibility Identification Experiment}
\label{sec:LowCredibilityIdentification}
The Low Credibility Identification Experiment (LCIE) was designed to evaluate the capability of a model to identify credibility articles (credibility score $< 3$) 
