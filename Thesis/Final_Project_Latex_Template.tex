...
\Abstract{
\vspace{-1.5 cm}
Misinformation is not a new phenomenon but the popularity and ubiquity of social media has meant that the rate at which information is produced and spreads greatly outpaces our ability to evaluate whether it is correct and unbiased. This is especially important in health and healthcare, where misinformation can influence attitudes and health behaviours that can lead to harm. In this project, I aimed to develop a novel approach to predicting the credibility of information presented on web pages using machine learning. To do this, I trained a series of supervised machine learning classifiers to predict the credibility of vaccine-related web pages. I used a dataset including the text extracted from 3,348 vaccine-related web pages, of which 470 were labelled by experts for credibility using a credibility appraisal checklist with 7 criteria. When predicting which of the 7 criteria were satisfied by an article, the standard approaches outperformed the deep learning approach. When the objective was to distinguish low credibility articles from all others, the best performing classifier was able to correctly label 92\% of unseen examples. The results indicate that automating credibility appraisal is feasible and that my approach could be implemented in practical tools for mitigating the spread of vaccine-related misinformation.
...
\begin{table}[H]
	...
	\resizebox{\textwidth}{!}{%
		\begin{tabular}{|p{6cm}|p{2.6cm}|p{2.6cm}|p{2.6cm}|p{2.6cm}|p{2.6cm}|p{2.6cm}|}
			...
			\multirowcell{2}{\textbf{Criterion 1:} \\ \textbf{Use of evidence} } 
			& NB & 0.79 & 0.85 & 0.79 & 0.79 & 0.82 \\ 
			& SVM & 0.82 & 0.78 & 0.80 & \textbf{0.86} & 0.60 \\ 
			\hline
			
			
			\multirowcell{2}{\textbf{Criterion 2:} \\ \textbf{Based on objective research}} & NB & 0.70 & 0.79 & 0.74 & 0.80 & 0.78 \\ 
...
In this chapter, I discuss the feasibility of automating credibility appraisal for vaccine-related articles online.  I start by summarising what I found through the set of experiments comparing different machine learning methods, and compare the results to what has been found in other document classification tasks in other application domains (Section 5.1). I then discuss how the novel methods I developed here might be used in real world applications aimed at mitigating the spread of vaccine-related information online (Section 5.2).
...
This thesis investigated the effectiveness and feasibility in employing ML and DL-based models on automating the process of evaluating the credibility of online articles, specifically articles related to vaccination. From the experiments conducted, it was determined that, for the current state of the project with its limitations, DL-based models were neither effective nor feasible for this task due to the relatively extremely limited amount of data available leaving the models unable to learn and generalise to a feasible degree. ML-based models using standard feature representations and NB and SVM classifiers however, showed promising signs on its capability to identify and classify select credibility criteria and SVM classifiers trained on TF-IDF scores showed to great effect its ability on differentiating between and classifying low credibility and high credibility criteria.
