...
The seven criteria defined in Table \ref{table:Criteria}, which was developed by a team of three health professionals for a separate and unpublished research project, will be used to define the structure and content that a vaccine-related health article must have to be considered as a credible source of information. \newline

The development and selection of the criteria was largely inspired by the tools and checklists discussed in Section \ref{sec:AssessingInformation} which has been adapted specifically for the evaluation of online articles pertaining to vaccines. However, extending this work for evaluating the credibility of articles in a different area or in a general case should remain feasible due to the novelty and specificity of the criteria being used provided that the criteria is adapted for the new classification task. \newline

Prior to the creation of the dataset used for this project, a small pilot experiment was conducted between the team that developed the criteria to ensure that the criteria was simple and objective enough such that the labelling of the articles remained consistent and reliable. The pilot experiment consisted of the labelling of the same \textbf{\textit{\#Num-articles}} articles by all three members of the team.
...
For the purposes of training and evaluating the performance of the baseline and deep learning models for this novel classification task, a dataset consisting of \textbf{\textit{1 Billion}} articles that have been manually and expertly labelled based on whether the article satisfies each criteria outlined in Table \ref{table:Criteria} has been developed. Due to the expert-level knowledge required to manually label the articles and the training to develop the knowledge required being out of the scope of this project, the articles within the dataset used were manually labelled by the same team that developed the credibility criteria as introduced in Section \ref{sec:CredibilityCriteria}.
...
\begin{enumerate}[-]
	\item This section should discuss the work proposed by Howard et al. and go into the limitations, advantages of this specific technique and how/why it is applicable in the context of this project
\end{enumerate}
