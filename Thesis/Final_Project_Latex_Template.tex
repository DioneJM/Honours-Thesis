...
Common datasets for text classification can be found under the 5 papers of interest doc. \newline

For DL language models, a common one is wikitext103 but there might be more in the paper by Jeremy Howard \newline

For the task of text classification, there are commonly used, open datasets for specific classification tasks that aim to provide a benchmark in which the performance of different implementations can be compared and to facilitate the data needs of deep learning models. Table \ref{table:Datasets} contains the commonly used datasets for various text classification tasks.

\begin{center}
	\begin{table}[!htb]
	\begin{tabular}{| l | p{6cm} | l | p{5cm} |}
		\hline
		Dataset & Description & Type & Objective \\ \hline
		SST & Movie reviews from Stanford Sentiment Treebank & Sentence & Sentiment \\ \hline
		
		IMDB & Internet Movie Database & Document & Sentiment \\ \hline
		
		MDSD & Product Reviews & Document & Sentiment \\ \hline
		
		RN & Reuters Newswire topics classification & Document & Topics \\ \hline
		
		QC & Question Classification & Sentence & Question Types \\ \hline
	\end{tabular}
	\caption{Commonly used classification datasets}
