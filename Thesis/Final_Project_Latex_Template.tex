...
Deep learning models are a class of machine learning models that have the capability of automatically learning a hierarchical representation of data \cite{Basheer2000}. These hierarchical representations are constructed through the use of artificial neural networks, the main underlying mechanism of deep learning models. The most commonly used models for document classification are Recurrent Neural Networks (RNNs), Long Short-Term Memory Networks (LSTMs) - a variant of RNNs and Convolutional Neural Networks (CNNs). RNNs and LSTMs are commonly used for document classification, or NLP tasks in general, due to their ability to create document representations that are able to capture the context and relationships of words within documents at a much more sophisticated level when compared to traditional features such as TF-IDF or n-grams \cite{Young}. 

\subsubsection{Recurrent Neural Networks}
\label{sec:RNN}

RNNs are popular choice for solving NLP tasks due to their ability in capturing the sequential nature of text at the character, word or sentence level. This allows RNNs to have the ability to create language models that account for the semantical meaning of words based on the previously occurring words in the sentence, allowing models to be capable of differentiating between words such as understanding that the word "dog" is likely referring to the animal whereas "hot dog" is more likely to refer to food. Factoring RNNs' capability in working with variable length inputs (e.g. long sentences, paragraphs or documents), they have shown to produce state-of-the-art results in document classification tasks \cite{Howard2018}. LSTMs \cite{Hochreiter1997} is a specific variant of RNNs that further improve on it by solving the vanishing and exploding gradient problem \cite{Socher} that feedforward networks i.e. RNNs are prone to.

\subsubsection{Convolutional Neural Networks}
\label{sec:CNN}

CNNs have traditionally seen extensive use for image classification tasks. However, with the work accomplished by Collbert et al. \cite{Collobert2011} who proposed a CNN-based model designed to solve various NLP tasks by extracting salient n-grams from the input text. 


\subsection{Classifying Documents with Limited Labelled Examples}
\label{sec:TransferLearning}
Typically, large amounts of training data is required to train a model in learning the language model required to attain state of the art results, in the task of document classification for instance, the size of commonly used non-domain specific datasets range from hundreds of thousands of training examples to millions \cite{Conneau2017} \cite{Zhang}. Due to these constraints, it is not feasible to procure a dataset for the domain specific task of this project due to the aforementioned knowledge expertise and time requirements to manually label the articles required. Hence, \textit{(Talk about transfer learning/N-shot learning/domain adaptation here)} will be used to overcome this issue.
